...
def get_llama_3_2_instruct_1B(folder_path, use_kv_cache=True, enable_dynamic_shape=True, max_seq_len=256, max_batch_size=1, generate_full_logit=False):
    root = folder_path

    #load json 
    config_path = os.path.join(root, "params.json")
    checkpoint_path = os.path.join(root, "consolidated.00.pth")

    with open(config_path, 'r') as f:
        config_dict = json.load(f)

    config_dict["use_kv_cache"] = use_kv_cache
    config_dict["max_seq_len"] = max_seq_len
    config_dict["max_batch_size"] = max_batch_size
    config_dict["enable_dynamic_shape"] = enable_dynamic_shape
    config_dict["generate_full_logits"] = generate_full_logit
